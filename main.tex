...
\label{sec:orgdd80c2f}

\section{Neurone Artificiale}
\label{sec:org652f4d6}
...
\label{sec:org283247e}

\subsubsection{Threshold step function}
\label{sec:org576f6da}
...
\label{sec:org932e75b}

\subsubsection{Gaussiana}
\label{sec:orga287289}
...
\label{sec:org2a2f4d5}
Per evitare il fenomeno dell'\href{../../../../../org/roam/20250627103519-overfitting.org}{overfitting}, è bene mantenere i parametri \uline{piccoli}. Pertanto, data una \hyperref[sec:org7e410c1]{funzione costo} \(C(\bm{w})\), la si \uline{regolarizza}, utilizzando una funzione costo \(G(\bm{w})\), data da \(C(\bm{w})\) più un termine di regolarizzazione.
...
\label{sec:orgbc14afe}
L'apprendimento di una \hyperref[sec:orge70a756]{rete neurale} è il processo di ricerca dei parametri ottimali per approssimare la funzione target. Questo è un processo iterativo algoritmico, che genera una \href{../../../../../org/roam/20250206170922-sequenze_e_stringhe.org}{sequenza} \((w_{t})\) di parametri. Poiché si parla di numeri immensi di elementi in questa sequenza, spesso ci si riferisce a \(t\) come una sorta di variabile temporale continua.
...
Si noti, inoltre, che se \(\sigma\) è la \hyperref[sec:org21c5504]{sigmoide} logistica, allora la derivata massima è ottenuto in \(x=0\) e vale
\begin{equation*}
...
\label{sec:org596fd8b}

\begin{prop}
Per ogni \href{../../../../../org/roam/20250630103918-funzione_periodica.org}{funzione periodica} \(F:\R\to \R\) esiste una \hyperref[sec:orge70a756]{rete neurale} che la approssima.
\end{prop}
...
\begin{itemize}
\item input: \(x \in \R\);
\item un \hyperref[sec:orgf296121]{layer nascosto} con \(N\) neuroni con funzione di attivazione \(\cos\), con peso dall'input \(w_{j}\) e bias \(b_{j}\);
...
\begin{thm}
Sia \(f \in L^{1}(\R)\)\footnote{Vedi ``\href{../../../../../org/roam/20250624162220-spazi_lp.org}{Spazi Lp}''\label{org44b1021}}. Lo \href{../../../../../org/roam/20250630122400-span.org}{span} di \(\set{T_{\theta}f\mid \theta \in \R}\) è \href{../../../../../org/roam/20250301193045-sottoinsieme_denso.org}{denso} in \(L^{1}(\R)\) se e solo se la \href{../../../../../org/roam/20250630121906-trasformata_di_fourier.org}{trasformata di Fourier}:
\begin{equation*}
...
La funzione \(G(x)\) è l'output di una \hyperref[sec:orge70a756]{rete neurale} con un \hyperref[sec:orgf296121]{layer nascosto} di \(N\) neuroni (e neurone di output lineare), dove i \hyperref[sec:org652f4d6]{neuroni} del layer nascosto hanno funzione di attivazione \(f\).
...
Si noti che le seguenti \hyperref[sec:org21c5504]{funzioni di attivazione} soddisfano le ipotesi dei Teoremi \ref{teo:wt1} e \ref{teo:wt2}.
\begin{description}
...
\label{sec:org567d896}

\section{Risultati preliminari}
\label{sec:orgf10c153}
