...
\label{sec:org71a05d6}

\subsubsection{Funzione Logistica}
\label{sec:orgdb6d067}
...
\label{sec:org0c25ef5}

\subsubsection{Gaussiana}
\label{sec:org4a32f3e}
...
\end{equation*}
dove \(\operatorname{ReLU}\) è la versione multidimensionale della \hyperref[sec:org4d22c92]{funzione di attivazione ReLU}, ovvero
\begin{equation*}
...
\label{sec:org002b9ac}
Per evitare il fenomeno dell'\href{../../../../../org/roam/20250627103519-overfitting.org}{overfitting}, è bene mantenere i parametri \uline{piccoli}. Pertanto, data una \hyperref[sec:org1b3d1fb]{funzione costo} \(C(\bm{w})\), la si \uline{regolarizza}, utilizzando una funzione costo \(G(\bm{w})\), data da \(C(\bm{w})\) più un termine di regolarizzazione.
...
\label{sec:org8a5b276}
L'apprendimento di una \hyperref[sec:orgbb3f8c2]{rete neurale} è il processo di ricerca dei parametri ottimali per approssimare la funzione target. Questo è un processo iterativo algoritmico, che genera una \href{../../../../../org/roam/20250206170922-sequenze_e_stringhe.org}{sequenza} \((w_{t})\) di parametri. Poiché si parla di numeri immensi di elementi in questa sequenza, spesso ci si riferisce a \(t\) come una sorta di variabile temporale continua.
...
\begin{definizione}
Il \uline{percettrone} è il neurone con \hyperref[sec:org594e902]{funzione di attivazione} \(H\).
\end{definizione}
\subsection{Interpretazione Geometrica}
\label{sec:orge49d4e8}
...
Si noti, inoltre, che se \(\sigma\) è la \hyperref[sec:org594e902]{sigmoide} logistica, allora la derivata massima è ottenuto in \(x=0\) e vale
\begin{equation*}
...
\begin{thm}
Sia \(N\ge {1}\) un intero fissato, e si consideri una \hyperref[sec:orgbb3f8c2]{rete neurale} con un \hyperref[sec:orgeecc0b4]{layer nascosto} tale che:
\begin{enumerate}
\item l'input della rete sia una variabile reale \(x \in [a,b]\);
\item l'output della rete sia un \hyperref[sec:org779563c]{neurone} unidimensionale con \hyperref[sec:org594e902]{funzione di attivazione lineare} e zero bias;
\item ci sono \(N\) neuroni nel layer nascosti con una funzione di attivazione differenziabile tale che \(|\sigma'|<\lambda<1\);
\item i pesi soddisfano la condizione di \hyperref[sec:org002b9ac]{regolarizzazione}:
\begin{equation*}
...
\begin{prop}
Anche per una \hyperref[sec:orgbb3f8c2]{rete neurale} composta da un unico \hyperref[sec:org779563c]{neurone}, con output
\begin{equation*}
f_{\bm{w},b}(\bm{x}) = \sigma(\bm{w}\cdot\bm{x} + b)
\end{equation*}
dove \(\sigma\) è la \hyperref[sec:orgdb6d067]{funzione logistica}, con pesi \(\bm{w} \in \R^{n}, b \in \R\) e input \(\bm{x} \in I_{n} \coloneqq [0,1]^{n}\), esiste una \href{../../../../../org/roam/20250103103252-funzione_continua.org}{funzione continua} \(g:I^{n}\to \R\) che può essere approssimata. Si supponga che \(\norma{\bm{w}}\le 1\).
\end{prop}
...
\label{sec:org05cc025}

\begin{prop}
Per ogni \href{../../../../../org/roam/20250630103918-funzione_periodica.org}{funzione periodica} \(F:\R\to \R\) esiste una \hyperref[sec:orgbb3f8c2]{rete neurale} che la approssima.
\end{prop}
...
\begin{itemize}
\item input: \(x \in \R\);
\item un \hyperref[sec:orgeecc0b4]{layer nascosto} con \(N\) neuroni con funzione di attivazione \(\cos\), con peso dall'input \(w_{j}\) e bias \(b_{j}\);
...
\begin{thm}
Sia \(f \in L^{1}(\R)\)\footnote{Vedi ``\href{../../../../../org/roam/20250624162220-spazi_lp.org}{Spazi Lp}''\label{org6bbe8bc}}. Lo \href{../../../../../org/roam/20250630122400-span.org}{span} di \(\set{T_{\theta}f\mid \theta \in \R}\) è \href{../../../../../org/roam/20250301193045-sottoinsieme_denso.org}{denso} in \(L^{1}(\R)\) se e solo se la \href{../../../../../org/roam/20250630121906-trasformata_di_fourier.org}{trasformata di Fourier}:
\begin{equation*}
...
La funzione \(G(x)\) è l'output di una \hyperref[sec:orgbb3f8c2]{rete neurale} con un \hyperref[sec:orgeecc0b4]{layer nascosto} di \(N\) neuroni (e neurone di output lineare), dove i \hyperref[sec:org779563c]{neuroni} del layer nascosto hanno funzione di attivazione \(f\).
...
Si noti che le seguenti \hyperref[sec:org594e902]{funzioni di attivazione} soddisfano le ipotesi dei Teoremi \ref{teo:wt1} e \ref{teo:wt2}.
\begin{description}
...
\label{sec:org255c163}

\section{Risultati preliminari}
\label{sec:orgbfcc21c}
...
\begin{oss}
La funzione \(c(x)\) di cui sopra è la funzione output di una \hyperref[sec:orgbb3f8c2]{rete neurale} con un layer nascosto:
\begin{itemize}
...
\begin{oss}
In particolare, tutte le \hyperref[sec:org594e902]{funzioni di attivazione} \href{../../../../../org/roam/20250625110110-funzione_sigmoidale.org}{sigmoidali} \(\varphi\) rispettano le ipotesi.
\end{oss}
...
\label{sec:org179d151}

\begin{lem}
La \hyperref[sec:org594e902]{funzione di attivazione} \emph{\hyperref[sec:orgf6dc227]{softplus}} \(\operatorname{sp}(x)\) è data dalla \href{../../../../../org/roam/20250703105424-prodotto_di_convoluzione.org}{convoluzione}:\footnote{Vedi la \hyperref[sec:org4d22c92]{funzione \(\operatorname{ReLU}\)}}
\begin{equation*}
\operatorname{sp}(x) = \convolution{\operatorname{ReLU}}{K}(x) = \int_{-\infty}^{+\infty} \operatorname{ReLU}(\tau) K(x-\tau)\dif\tau
\end{equation*}
dove \(K(x) = \frac{1}{(1+e^{x})(1+e^{-x})} = \sigma'(x)\)\footnote{Dove \(\sigma(x)\) è la \hyperref[sec:orgdb6d067]{Funzione Logistica}.}.
\end{lem}
...
\begin{lem}
Si definisca ora la \hyperref[sec:orgf6dc227]{funzione softplus} scalata:
\begin{equation*}
...
\begin{lem}
Sia \(I_{n}\coloneqq[0,1]^{n}\), e si consideri \(C(I_{n})\)\footnote{Vedi ``\href{../../../../../org/roam/20250113125602-classe_c_di_una_funzione.org}{Classe C di una funzione}''\label{orgaf93b1c}} dotato della \href{../../../../../org/roam/20250103145124-topologia.org}{topologia} \href{../../../../../org/roam/20250301193530-topologia_indotta_da_una_distanza.org}{indotta} dalla \href{../../../../../org/roam/20250301193511-spazio_metrico.org}{metrica}
\begin{equation*}
...
\end{equation*}
Si vedano ``\href{../../../../../org/roam/20250624162220-spazi_lp.org}{Spazi Lp}''\label{orga05391e}} e
\begin{equation*}
...
\end{prop}
\begin{thm}
Ogni funzione in \(L^{q}(\R^{n})\)\footnote{Vedi ``\href{../../../../../org/roam/20250624162220-spazi_lp.org}{Spazi Lp}''} può essere approssimata arbitrariamente bene nella norma \(\norma{\cdot}_{q}\) da una \hyperref[sec:orgbb3f8c2]{rete neurale} \hyperref[sec:orgac723aa]{feedforward} \hyperref[sec:org4d22c92]{ReLU}.
\end{thm}
\subsection{Accuratezza dell'approssimazione}
\label{sec:org28696e7}

Si consideri una \hyperref[sec:orgbb3f8c2]{rete neurale} con un \hyperref[sec:orgeecc0b4]{layer nascosto} contenente \(N\) \hyperref[sec:org779563c]{neuroni}, che produce un output \(G(x)\), che vuole \href{../../../../../org/roam/20250630103725-funzioni_approssimate_da_una_rete_neurale.org}{approssimare} \(f\) in uno spazio funzionale quale\footnote{Sia \(I_{n}\coloneqq[0,1]^{n}\). Vedi
\begin{itemize}
...
\label{sec:org7cd6007}

\section{Exact Learning (Machine Learning)}
\label{sec:org0a7b3a7}
By exact learning we mean the expressibility of a \hyperref[sec:orgbb3f8c2]{network} to reproduce exactly the desired target function, i.e. the output of the network is exactly the target function. For an exact learning the network weights do not need \hyperref[sec:org8a5b276]{tuning}; their values can be found exactly. Si parla quindi di \uline{rappresentazione esatta}, e non di \uline{apprendimento}.
...
\label{sec:org0044905}
Si consideri a titolo esplicativo \(C([0,1])\)\footnote{Vedi ``\href{../../../../../org/roam/20250113125602-classe_c_di_una_funzione.org}{Classe C di una funzione}''}, e si consideri una \hyperref[sec:orgbb3f8c2]{rete neurale} con un \hyperref[sec:orgeecc0b4]{layer nascosto} con \hyperref[sec:org594e902]{funzione di attivazione} \hyperref[sec:orgdb6d067]{logistica} \(\sigma\). La \hyperref[sec:orgbb3f8c2]{funzione di input-output della rn} è
\begin{equation*}
...
\label{sec:org06104f9}

\begin{prop}
La funzione \(\max\set{x_{1},x_{2}}\) è \hyperref[sec:org0a7b3a7]{rappresentata esattamente} da una \hyperref[sec:orgbb3f8c2]{rete neurale} con un \hyperref[sec:orgeecc0b4]{layer nascosto} con 4 \hyperref[sec:org779563c]{neuroni} \hyperref[sec:org4d22c92]{ReLU}, ovvero
\begin{equation*}
...
\begin{prop}
La funzione \(f(x_{1},\dots,f_{x_{n}}) = \max\set{x_{1},\dots,x_{n}}\) è \hyperref[sec:org0a7b3a7]{rappresentata esattamente} da una \hyperref[sec:orgbb3f8c2]{rete neurale} \href{../../../../../org/roam/20250624155858-neurone_artificiale.org}{ReLU-feedforward} con
\begin{equation*}
L=\begin{cases}
2k & \text{se }n=2^{k}\\
...
\begin{prop}
Esiste una \hyperref[sec:orgbb3f8c2]{rete neurale} \hyperref[sec:orgac723aa]{feedforward} con input di dimensione 2, un layer nascosto di larghezza 2, output di dimensione 1 con \hyperref[sec:org594e902]{funzione di attivazione} \hyperref[sec:org4d22c92]{ReLU} in ogni \hyperref[sec:org779563c]{neurone} che \hyperref[sec:org0a7b3a7]{rappresenta esattamente} la funzione
\begin{equation*}
