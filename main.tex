...
\label{sec:org5001e86}

\section{Neurone Artificiale}
\label{sec:org3b88f09}
...
\label{sec:orge12b133}
Nel Machine Learning le funzioni che agiscono nei \hyperref[sec:org3b88f09]{neuroni} vengono dette \uline{funzioni di attivazione}. Se ne presentano alcuni esempi, con i loro nomi specifici.

Sono tutte funzioni \(A \subseteq\R\to B \subseteq \R\).
\subsection{Funzioni Lineari}
\label{sec:orgbb7569c}
...
\label{sec:org5ff9437}

\subsubsection{Threshold step function}
\label{sec:orgd0b27e9}
...
\label{sec:org7aaa28b}

\subsubsection{Funzione Logistica}
\label{sec:orgd0a8168}
...
Spesso ci si rifesce a \(\sigma\coloneqq \sigma_{1}\) come alla \uline{funzione logistica} o \uline{funzione sigmoide} (in quanto è l'archetipo della \href{../../../../../org/roam/20250625110110-funzione_sigmoidale.org}{Funzione sigmoidale}).
\subsubsection{Tangente Iperbolica}
\label{sec:orgd042e4f}
...
\label{sec:org562b40c}

\subsubsection{Gaussiana}
\label{sec:org7941c14}
...
\begin{itemize}
\item \(L-1\) \href{../../../../../org/roam/20250202130045-insieme_dei_numeri_naturali_mk.org}{numeri naturali} \(\ell_{1},\dots,\ell_{L-1}\);
\item \(L\) \href{../../../../../org/roam/20250129094132-trasformazione_affine.org}{funzioni affini} \(A_{1},\dots,A_{L}\) tali che
\begin{align*}
  A_{1}:\R^{n}&\to \R^{\ell_{1}}\\
...
\end{equation*}
dove \(\operatorname{ReLU}\) è la versione multidimensionale della \hyperref[sec:orge0d0a6d]{funzione di attivazione ReLU}, ovvero
\begin{equation*}
...
\label{sec:org3e24f28}
Per evitare il fenomeno dell'\href{../../../../../org/roam/20250627103519-overfitting.org}{overfitting}, è bene mantenere i parametri \uline{piccoli}. Pertanto, data una \hyperref[sec:org54c6538]{funzione costo} \(C(\bm{w})\), la si \uline{regolarizza}, utilizzando una funzione costo \(G(\bm{w})\), data da \(C(\bm{w})\) più un termine di regolarizzazione.
...
\label{sec:org35fab32}
L'apprendimento di una \hyperref[sec:org599fe74]{rete neurale} è il processo di ricerca dei parametri ottimali per approssimare la funzione target. Questo è un processo iterativo algoritmico, che genera una \href{../../../../../org/roam/20250206170922-sequenze_e_stringhe.org}{sequenza} \((w_{t})\) di parametri. Poiché si parla di numeri immensi di elementi in questa sequenza, spesso ci si riferisce a \(t\) come una sorta di variabile temporale continua.
...
Si noti, inoltre, che se \(\sigma\) è la \hyperref[sec:orge12b133]{sigmoide} logistica, allora la derivata massima è ottenuto in \(x=0\) e vale
\begin{equation*}
...
\begin{thm}
Sia \(N\ge {1}\) un intero fissato, e si consideri una \hyperref[sec:org599fe74]{rete neurale} con un \hyperref[sec:org5035087]{layer nascosto} tale che:
\begin{enumerate}
\item l'input della rete sia una variabile reale \(x \in [a,b]\);
\item l'output della rete sia un \hyperref[sec:org3b88f09]{neurone} unidimensionale con \hyperref[sec:orge12b133]{funzione di attivazione lineare} e zero bias;
\item ci sono \(N\) neuroni nel layer nascosti con una funzione di attivazione differenziabile tale che \(|\sigma'|<\lambda<1\);
\item i pesi soddisfano la condizione di \hyperref[sec:org3e24f28]{regolarizzazione}:
\begin{equation*}
...
\begin{prop}
Anche per una \hyperref[sec:org599fe74]{rete neurale} composta da un unico \hyperref[sec:org3b88f09]{neurone}, con output
\begin{equation*}
f_{\bm{w},b}(\bm{x}) = \sigma(\bm{w}\cdot\bm{x} + b)
\end{equation*}
dove \(\sigma\) è la \hyperref[sec:orgd0a8168]{funzione logistica}, con pesi \(\bm{w} \in \R^{n}, b \in \R\) e input \(\bm{x} \in I_{n} \coloneqq [0,1]^{n}\), esiste una \href{../../../../../org/roam/20250103103252-funzione_continua.org}{funzione continua} \(g:I^{n}\to \R\) che può essere approssimata. Si supponga che \(\norma{\bm{w}}\le 1\).
\end{prop}
...
\label{sec:org6bfee91}

\begin{prop}
Per ogni \href{../../../../../org/roam/20250630103918-funzione_periodica.org}{funzione periodica} \(F:\R\to \R\) esiste una \hyperref[sec:org599fe74]{rete neurale} che la approssima.
\end{prop}
...
L'insieme \(\mathcal{A} \subseteq C([0,T])\)\footnote{Vedi ``\href{../../../../../org/roam/20250113125602-classe_c_di_una_funzione.org}{Classe C di una funzione}''}, ed inoltre è una \href{../../../../../org/roam/20250629165520-algebra_di_funzioni_reali.org}{\(\R\)-algebra}\footnote{Questo si vede facilmente seguendo l'Esempio di ``\href{../../../../../org/roam/20250629165520-algebra_di_funzioni_reali.org}{Algebra di funzioni reali}''}. Contiene tutte le funzioni costanti (basta porre \(a_{j}=c_{j} = 0\) per ogni \(j>0\)), e separa i punti, in quanto
\begin{equation*}
...
\begin{itemize}
\item input: \(x \in \R\);
\item un \hyperref[sec:org5035087]{layer nascosto} con \(N\) neuroni con funzione di attivazione \(\cos\), con peso dall'input \(w_{j}\) e bias \(b_{j}\);
...
La funzione \(G(x)\) è l'output di una \hyperref[sec:org599fe74]{rete neurale} con un \hyperref[sec:org5035087]{layer nascosto} di \(N\) neuroni (e neurone di output lineare), dove i \hyperref[sec:org3b88f09]{neuroni} del layer nascosto hanno funzione di attivazione \(f\).
...
Si noti che le seguenti \hyperref[sec:orge12b133]{funzioni di attivazione} soddisfano le ipotesi dei Teoremi \ref{teo:wt1} e \ref{teo:wt2}.
\begin{description}
...
\label{sec:org29f724c}

\section{Risultati preliminari}
\label{sec:org004e20e}
...
\begin{oss}
La funzione \(c(x)\) di cui sopra è la funzione output di una \hyperref[sec:org599fe74]{rete neurale} con un layer nascosto:
\begin{itemize}
...
\end{itemize}
\end{oss}
\subsection{One Hidden Layer Sigmoidal Network}
\label{sec:org7200202}
...
Sia \(g \in C[0,1]\)\footnote{Vedi ``\href{../../../../../org/roam/20250113125602-classe_c_di_una_funzione.org}{Classe C di una funzione}''}. Allora per ogni \(\varepsilon>0\) esiste \(N \in \N\) ed esistono, per ogni \(i=0,\dots,N\) degli \(c_{i}, \theta_{i}, w \in \R\) tali che
\begin{equation*}
...
\begin{oss}
In particolare, tutte le \hyperref[sec:orge12b133]{funzioni di attivazione} \href{../../../../../org/roam/20250625110110-funzione_sigmoidale.org}{sigmoidali} \(\varphi\) rispettano le ipotesi.
\end{oss}
...
\end{proof}
\begin{thm}
Si consideri la \hyperref[sec:orge12b133]{funzione di attivazione} \(\operatorname{ReLU}(x) = x\,H(x)\)\footnote{Dove \(H(x)\) è la \href{../../../../../org/roam/20250624161413-funzione_di_heaviside.org}{Funzione di Heaviside}}. Allora per ogni \(g \in C[0,1]\)\footnote{Vedi ``\href{../../../../../org/roam/20250113125602-classe_c_di_una_funzione.org}{Classe C di una funzione}''} esiste \(N \in \N\) ed esistono, per \(i=0,\dots,N-1\) degli \(\alpha_{i}, \theta_{i}, \beta \in \R\) tali che, detta
\begin{equation*}
...
\label{sec:org3430f60}

\begin{lem}
La \hyperref[sec:orge12b133]{funzione di attivazione} \emph{\hyperref[sec:org0492845]{softplus}} \(\operatorname{sp}(x)\) è data dalla \href{../../../../../org/roam/20250703105424-prodotto_di_convoluzione.org}{convoluzione}:\footnote{Vedi la \hyperref[sec:orge0d0a6d]{funzione \(\operatorname{ReLU}\)}}
\begin{equation*}
\operatorname{sp}(x) = \convolution{\operatorname{ReLU}}{K}(x) = \int_{-\infty}^{+\infty} \operatorname{ReLU}(\tau) K(x-\tau)\dif\tau
\end{equation*}
dove \(K(x) = \frac{1}{(1+e^{x})(1+e^{-x})} = \sigma'(x)\)\footnote{Dove \(\sigma(x)\) è la \hyperref[sec:orgd0a8168]{Funzione Logistica}.}.
\end{lem}
...
\begin{lem}
Si definisca ora la \hyperref[sec:org0492845]{funzione softplus} scalata:
\begin{equation*}
...
\begin{thm}
Sia \(g \in C[0,1]\)\footnote{Vedi ``\href{../../../../../org/roam/20250113125602-classe_c_di_una_funzione.org}{Classe C di una funzione}''}. Allora per ogni \(\varepsilon>0\) esiste \(N \in \N\) ed esistono, per ogni \(i=0,\dots,N-1\), degli \(c_{j},w,\theta_{j},\beta \in \R\) tali che
\begin{equation*}
...
\begin{lem}
Sia \(I_{n}\coloneqq[0,1]^{n}\), e si consideri \(C(I_{n})\)\footnote{Vedi ``\href{../../../../../org/roam/20250113125602-classe_c_di_una_funzione.org}{Classe C di una funzione}''\label{org0447f50}} dotato della \href{../../../../../org/roam/20250103145124-topologia.org}{topologia} \href{../../../../../org/roam/20250301193530-topologia_indotta_da_una_distanza.org}{indotta} dalla \href{../../../../../org/roam/20250301193511-spazio_metrico.org}{metrica}
\begin{equation*}
d(f,g) \coloneqq \sup_{x \in I_{n}} |f(x)-g(x)| = \max_{x \in I_{n}}|f(x)-g(x)|.
\end{equation*}
Sia \(\mathcal{M}\) la famiglia delle \href{../../../../../org/roam/20250625104200-misura_di_baire.org}{misure di Baire} per \(\R^{n}\) sul cubo \(I^{n}\R\), \href{../../../../../org/roam/20250625110016-misura_finita.org}{finite}, \href{../../../../../org/roam/20250625110024-misura_con_segno.org}{con segno} e \href{../../../../../org/roam/20250625110032-misura_regolare.org}{regolari}.

Sia \(\mathcal{U} \subseteq C(I_{n})\) un \href{../../../../../org/roam/20250114103118-sottospazio_vettoriale.org}{sottospazio vettoriale} non \href{../../../../../org/roam/20250301193045-sottoinsieme_denso.org}{denso}. Allora esiste una \href{../../../../../org/roam/20250704163455-misura.org}{misura} \(\mu \in \mathcal{M}\) tale che
\begin{equation*}
...
\begin{prop}
Sia \(\sigma\) una \href{../../../../../org/roam/20250625105528-funzione_discriminatoria_per_una_misura_di_baire_sul_cubo_unitario.org}{funzione discriminatoria} e \href{../../../../../org/roam/20250103103252-funzione_continua.org}{continua}. Allora l'insieme
\begin{equation*}
...
\begin{thm}
Sia \(I_{n}\coloneqq[0,1]^{n}\), e si consideri \(C(I_{n})\)\footnote{Vedi ``\href{../../../../../org/roam/20250113125602-classe_c_di_una_funzione.org}{Classe C di una funzione}''} dotato della \href{../../../../../org/roam/20250103145124-topologia.org}{topologia} \href{../../../../../org/roam/20250301193530-topologia_indotta_da_una_distanza.org}{indotta} dalla \href{../../../../../org/roam/20250301193511-spazio_metrico.org}{metrica}
\begin{equation*}
d(f,g) \coloneqq \sup_{x \in I_{n}} |f(x)-g(x)| = \max_{x \in I_{n}}|f(x)-g(x)|.
\end{equation*}

Sia \(\varphi: \R\to \R\) una qualsiasi \href{../../../../../org/roam/20250103103252-funzione_continua.org}{funzione continua} non costante. Allora le funzioni nella forma
\begin{equation*}
G(x) = \sum_{k=1}^{M} \beta_{k}\prod_{j=1}^{N_{k}}\varphi(\bm{w}_{jk}\cdot \bm{x} + \theta_{jk}),\qquad G:I_{n}\to \R
\end{equation*}
con \(\bm{w}_{jk} \in \R^{n}\), \(\beta_{j},\theta_{jk} \in \R\), \(M,N_{k} \in \N\), sono \href{../../../../../org/roam/20250301193045-sottoinsieme_denso.org}{dense} in \(C(I_{n})\)
\label{teo9.3.8}
\end{thm}
\subsection{Funzioni \(L^{2}\)}
\label{sec:org02fba9c}
...
\end{equation*}
Si vedano ``\href{../../../../../org/roam/20250624162220-spazi_lp.org}{Spazi Lp}''\label{org12e2fdf}} e
\begin{equation*}
...
\begin{prop}
Sia \(\sigma:\R\to\R\) una \href{../../../../../org/roam/20250202170607-classe_relazione_binaria.org}{funzione} \href{../../../../../org/roam/20250625105528-funzione_discriminatoria_per_una_misura_di_baire_sul_cubo_unitario.org}{discriminatoria in senso \(L^{2}\)}. Allora l'insieme
\begin{equation*}
\set{
G(\bm{x}) = \sum_{j=1}^{N}\alpha_{j}\sigma(\bm{w}_{j}\cdot \bm{x} + \theta_{j})\mid N \in \N, \bm{w}_{j} \in \R^{n}, \alpha_{j},\theta_{j} \in \R
}
\end{equation*}
è \href{../../../../../org/roam/20250301193045-sottoinsieme_denso.org}{denso} in \(L^{2}(I_{n})\)\footnote{Con \(I_{n}\) si indica il cubo
\begin{equation*}
I_{n} = [0,1]\times \dots \times [0,1] = [0,1]^{n}.
\end{equation*}
Si vedano ``\href{../../../../../org/roam/20250624162220-spazi_lp.org}{Spazi Lp}''}.
\label{prop9.3.11}
\end{prop}
...
I_{n} = [0,1]\times \dots \times [0,1] = [0,1]^{n}.
\end{equation*}
Si vedano ``\href{../../../../../org/roam/20250624162220-spazi_lp.org}{Spazi Lp}''}. Se per ogni \(\bm{w} \in \R^{n}\) e per ogni \(\theta \in \R\), detto \(\mathcal{H}_{\bm{w},\theta} \coloneqq \set{x \in I_{n}\mid \bm{w}\cdot\bm{x}+\theta>0}\) il semispazio, si ha che
\begin{equation*}
\int_{\mathcal{H}_{\bm{w},\theta}} g(\bm{x})\dif\bm{x} = 0
\end{equation*}
allora \(g=0\) \href{../../../../../org/roam/20250630122745-proprieta_vera_quasi_ovunque.org}{q.o.}
\label{lem9.3.12}
\end{lem}
\subsection{Funzioni \(L^{1}\)}
\label{sec:org82222b5}
...
\subsection{Funzioni \(L^{q}\)}
\label{sec:orgcea05d4}

\begin{prop}
La famiglia delle \href{../../../../../org/roam/20250202170607-classe_relazione_binaria.org}{funzioni} \href{../../../../../org/roam/20250103103252-funzione_continua.org}{continue} a \href{../../../../../org/roam/20250701115005-supporto_di_una_funzione.org}{supporto} \href{../../../../../org/roam/20250103163701-spazio_topologico_compatto.org}{compatto}
\begin{equation*}
\set{f:\R^{n}\to \R\mid f\text{ continua a supporto compatto}}
\end{equation*}
è \href{../../../../../org/roam/20250301193045-sottoinsieme_denso.org}{densa} in \(L^{q}(\R^{n})\) per ogni \(q \in [1,+ \infty)\)\footnote{Vedi ``\href{../../../../../org/roam/20250624162220-spazi_lp.org}{Spazi Lp}''}.
\end{prop}
\begin{prop}
La famiglia delle funzioni continue e \href{../../../../../org/roam/20250701140551-funzione_lineare_a_tratti.org}{lineari a tratti} \(\R^{n}\to \R\) è densa nella famiglia delle \href{../../../../../org/roam/20250202170607-classe_relazione_binaria.org}{funzioni} \href{../../../../../org/roam/20250103103252-funzione_continua.org}{continue} a \href{../../../../../org/roam/20250701115005-supporto_di_una_funzione.org}{supporto} \href{../../../../../org/roam/20250103163701-spazio_topologico_compatto.org}{compatto} con la \href{../../../../../org/roam/20250103145124-topologia.org}{topologia} \href{../../../../../org/roam/20250301193530-topologia_indotta_da_una_distanza.org}{indotta} dalla \href{../../../../../org/roam/20250301193511-spazio_metrico.org}{distanza}
\begin{equation*}
d(f,g) = \sup_{x \in \R^{n}}|f(x)-g(x)|
\end{equation*}
\end{prop}
\begin{thm}
Ogni funzione in \(L^{q}(\R^{n})\)\footnote{Vedi ``\href{../../../../../org/roam/20250624162220-spazi_lp.org}{Spazi Lp}''} può essere approssimata arbitrariamente bene nella norma \(\norma{\cdot}_{q}\) da una \hyperref[sec:org599fe74]{rete neurale} \hyperref[sec:org70441ed]{feedforward} \hyperref[sec:orge0d0a6d]{ReLU}.
\end{thm}
\subsection{Accuratezza dell'approssimazione}
\label{sec:orga7ca9f6}

Si consideri una \hyperref[sec:org599fe74]{rete neurale} con un \hyperref[sec:org5035087]{layer nascosto} contenente \(N\) \hyperref[sec:org3b88f09]{neuroni}, che produce un output \(G(x)\), che vuole \href{../../../../../org/roam/20250630103725-funzioni_approssimate_da_una_rete_neurale.org}{approssimare} \(f\) in uno spazio funzionale quale\footnote{Sia \(I_{n}\coloneqq[0,1]^{n}\). Vedi
\begin{itemize}
\item ``\href{../../../../../org/roam/20250113125602-classe_c_di_una_funzione.org}{Classe C di una funzione}'' e ``\href{../../../../../org/roam/20250706121659-ohldn_f_continue.org}{One Hidden Layer Discriminatory Network impara funzioni continue sul cubo}''.
\item Si consideri \(C(I_{n})\) dotato della \href{../../../../../org/roam/20250103145124-topologia.org}{topologia} \href{../../../../../org/roam/20250301193530-topologia_indotta_da_una_distanza.org}{indotta} dalla \href{../../../../../org/roam/20250301193511-spazio_metrico.org}{metrica}
\begin{equation*}
  d(f,g) \coloneqq \sup_{x \in I_{n}} |f(x)-g(x)| = \max_{x \in I_{n}}|f(x)-g(x)|.
\end{equation*}
\item ``\href{../../../../../org/roam/20250624162220-spazi_lp.org}{Spazi Lp}'' e
\begin{itemize}
\item ``\href{../../../../../org/roam/20250706121659-ohldnl1.org}{One Hidden Layer Discriminatory Network impara funzioni L1 sul cubo}''
\item ``\href{../../../../../org/roam/20250706121659-ohldnl2.org}{One Hidden Layer Discriminatory Network impara funzioni L2 sul cubo}''
\end{itemize}
\end{itemize}}
\begin{equation*}
...
\end{equation*}
\end{prop}
\chapter{Exact Approximation}
\label{sec:org202904f}

\section{Exact Learning (Machine Learning)}
\label{sec:orgc7d4890}
By exact learning we mean the expressibility of a \hyperref[sec:org599fe74]{network} to reproduce exactly the desired target function, i.e. the output of the network is exactly the target function. For an exact learning the network weights do not need \hyperref[sec:org35fab32]{tuning}; their values can be found exactly. Si parla quindi di \uline{rappresentazione esatta}, e non di \uline{apprendimento}.

Alcuni casi in cui questo può succedere:
\begin{itemize}
\item \href{../../../../../org/roam/20250708121822-one_hidden_layer_relu_network_impara_esattamente_funzioni_a_supporto_finito.org}{One Hidden Layer Perceptron Network rappresenta esattamente funzioni a supporto finito}
\item \href{../../../../../org/roam/20250708122516-rete_neurale_relu_feedforward_impara_esattamente_la_funzione_massimo.org}{Rete Neurale ReLU-feedforward rappresenta esattamente la funzione massimo tra N input}
\end{itemize}
\subsection{Exact Learning non è sempre possibile}
\label{sec:orgcdb4714}
Si consideri a titolo esplicativo \(C([0,1])\)\footnote{Vedi ``\href{../../../../../org/roam/20250113125602-classe_c_di_una_funzione.org}{Classe C di una funzione}''}, e si consideri una \hyperref[sec:org599fe74]{rete neurale} con un \hyperref[sec:org5035087]{layer nascosto} con \hyperref[sec:orge12b133]{funzione di attivazione} \hyperref[sec:orgd0a8168]{logistica} \(\sigma\). La \hyperref[sec:org599fe74]{funzione di input-output della rn} è
\begin{equation*}
G(x) = \sum_{j=1}^{N}\alpha_{j}\sigma(w_{j}x+\theta_{j}).
\end{equation*}

Siccome \(\sigma\) è \href{../../../../../org/roam/20250709150055-funzione_analitica.org}{analitica}, allora \(G(x)\) è analitica, mentre esistono funzioni continue ma non analitiche, come
\begin{equation*}
f(x)=\begin{cases}
...
\end{cases}
\end{equation*}
\section{Funzioni a supporto finito}
\label{sec:org6a3ea6c}
\begin{prop}
Sia \(g:\R^{r}\to \R\) una \href{../../../../../org/roam/20250202170607-classe_relazione_binaria.org}{funzione} qualsiasi e sia \(S=\set{\bm{x}_{1},\dots,\bm{x}_{n}} \subseteq \R^{r}\). Allora esiste
\begin{equation*}
G(\bm{x}) = \sum_{j=1}^{n} \alpha_{j} \, H(\bm{w}_{j}\cdot \bm{x}-\theta_{i}),
\end{equation*}
dove \(H\) è la \href{../../../../../org/roam/20250624161413-funzione_di_heaviside.org}{funzione di Heaviside}, \(\alpha_{j},\theta_{j} \in \R\) e \(\bm{w}_{j} \in \R^{r}\), tale che
\begin{equation*}
...
\label{cor10.1.3}
\end{cor}
\section{Funzione massimo}
\label{sec:orge544b5e}

\begin{prop}
La funzione \(\max\set{x_{1},x_{2}}\) è \hyperref[sec:orgc7d4890]{rappresentata esattamente} da una \hyperref[sec:org599fe74]{rete neurale} con un \hyperref[sec:org5035087]{layer nascosto} con 4 \hyperref[sec:org3b88f09]{neuroni} \hyperref[sec:orge0d0a6d]{ReLU}, ovvero
\begin{equation*}
...
\begin{prop}
La funzione \(f(x_{1},\dots,f_{x_{n}}) = \max\set{x_{1},\dots,x_{n}}\) è \hyperref[sec:orgc7d4890]{rappresentata esattamente} da una \hyperref[sec:org599fe74]{rete neurale} \href{../../../../../org/roam/20250624155858-neurone_artificiale.org}{ReLU-feedforward} con
\begin{equation*}
L=\begin{cases}
2k & \text{se }n=2^{k}\\
...
\begin{prop}
Esiste una \hyperref[sec:org599fe74]{rete neurale} \hyperref[sec:org70441ed]{feedforward} con input di dimensione 2, un layer nascosto di larghezza 2, output di dimensione 1 con \hyperref[sec:orge12b133]{funzione di attivazione} \hyperref[sec:orge0d0a6d]{ReLU} in ogni \hyperref[sec:org3b88f09]{neurone} che \hyperref[sec:orgc7d4890]{rappresenta esattamente} la funzione
\begin{equation*}
...
\begin{lem}
La \hyperref[sec:org599fe74]{funzione input-output} di una \hyperref[sec:org599fe74]{rete neurale} \href{../../../../../org/roam/20250624155858-neurone_artificiale.org}{ReLU-feedforward} è una \href{../../../../../org/roam/20250103103252-funzione_continua.org}{funzione continua} e \href{../../../../../org/roam/20250701140551-funzione_lineare_a_tratti.org}{lineare a tratti}.
\label{lem10.2.6}
\end{lem}
\begin{prop}
Sia \(f:\R^{n}\to \R\) una \href{../../../../../org/roam/20250103103252-funzione_continua.org}{funzione continua} e \href{../../../../../org/roam/20250701140551-funzione_lineare_a_tratti.org}{lineare a tratti}. Allora esiste una \hyperref[sec:org599fe74]{rete neurale} \hyperref[sec:org70441ed]{feedforward}, con \hyperref[sec:org3b88f09]{neuroni} \hyperref[sec:orge0d0a6d]{ReLU} e \hyperref[sec:orge12b133]{lineari}, che \hyperref[sec:orgc7d4890]{rappresenta esattamente} \(f\). Inoltre ha \(L\) \hyperref[sec:org5035087]{layers}, con
\begin{equation*}
\begin{equation*}
L=\begin{cases}
