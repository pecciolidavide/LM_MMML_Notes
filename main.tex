...
\label{sec:org06d49ca}

\section{Neurone Artificiale}
\label{sec:org5e0fef6}
...
\label{sec:orgb35bde6}
Nel Machine Learning le funzioni che agiscono nei \hyperref[sec:org5e0fef6]{neuroni} vengono dette \uline{funzioni di attivazione}. Se ne presentano alcuni esempi, con i loro nomi specifici.

Sono tutte funzioni \(A \subseteq\R\to B \subseteq \R\).
\subsection{Funzioni Lineari}
\label{sec:orgb6aa892}
...
\label{sec:org506669a}

\subsubsection{Threshold step function}
\label{sec:orgb4006bd}
...
\label{sec:org26ca9c5}

\subsubsection{ReLU}
\label{sec:orgec80353}
...
\label{sec:org6d76180}

\subsubsection{Funzione Logistica}
\label{sec:org542380f}
...
\label{sec:org9dd9083}

\subsubsection{Gaussiana}
\label{sec:orgb157c80}
...
\label{sec:org2510457}
Per evitare il fenomeno dell'\href{../../../../../org/roam/20250627103519-overfitting.org}{overfitting}, è bene mantenere i parametri \uline{piccoli}. Pertanto, data una \hyperref[sec:org5178eac]{funzione costo} \(C(\bm{w})\), la si \uline{regolarizza}, utilizzando una funzione costo \(G(\bm{w})\), data da \(C(\bm{w})\) più un termine di regolarizzazione.
...
\label{sec:org9590346}
L'apprendimento di una \hyperref[sec:orgb157329]{rete neurale} è il processo di ricerca dei parametri ottimali per approssimare la funzione target. Questo è un processo iterativo algoritmico, che genera una \href{../../../../../org/roam/20250206170922-sequenze_e_stringhe.org}{sequenza} \((w_{t})\) di parametri. Poiché si parla di numeri immensi di elementi in questa sequenza, spesso ci si riferisce a \(t\) come una sorta di variabile temporale continua.
...
Si noti, inoltre, che se \(\sigma\) è la \hyperref[sec:orgb35bde6]{sigmoide} logistica, allora la derivata massima è ottenuto in \(x=0\) e vale
\begin{equation*}
...
\begin{thm}
Sia \(N\ge {1}\) un intero fissato, e si consideri una \hyperref[sec:orgb157329]{rete neurale} con un \hyperref[sec:orga494515]{layer nascosto} tale che:
\begin{enumerate}
\item l'input della rete sia una variabile reale \(x \in [a,b]\);
\item l'output della rete sia un \hyperref[sec:org5e0fef6]{neurone} unidimensionale con \hyperref[sec:orgb35bde6]{funzione di attivazione lineare} e zero bias;
\item ci sono \(N\) neuroni nel layer nascosti con una funzione di attivazione differenziabile tale che \(|\sigma'|<\lambda<1\);
\item i pesi soddisfano la condizione di \hyperref[sec:org2510457]{regolarizzazione}:
\begin{equation*}
...
\end{proof}
\begin{prop}
Anche per una \hyperref[sec:orgb157329]{rete neurale} composta da un unico \hyperref[sec:org5e0fef6]{neurone}, con output
\begin{equation*}
f_{\bm{w},b}(\bm{x}) = \sigma(\bm{w}\cdot\bm{x} + b)
\end{equation*}
dove \(\sigma\) è la \hyperref[sec:org542380f]{funzione logistica}, con pesi \(\bm{w} \in \R^{n}, b \in \R\) e input \(\bm{x} \in I_{n} \coloneqq [0,1]^{n}\), esiste una \href{../../../../../org/roam/20250103103252-funzione_continua.org}{funzione continua} \(g:I^{n}\to \R\) che può essere approssimata. Si supponga che \(\norma{\bm{w}}\le 1\).
\end{prop}
...
\begin{prop}
Sia \(f:[a,b]\times[c,d]\to \R\) una funzione continua. Allora, per ogni \(\varepsilon>0\) esiste \(N\ge 1\) ed esistono, per ogni \(i=1,\dots,N\), delle \(g_{i} \in C([a,b])\)\textsuperscript{\ref{org83d3708}} e \(h_{i} \in C([c,d])\) tali che
\begin{equation*}
\max_{\substack{
x \in [a,b]\\
y \in [c,d]}}
...
\label{sec:org5b77dda}

\begin{prop}
Per ogni \href{../../../../../org/roam/20250630103918-funzione_periodica.org}{funzione periodica} \(F:\R\to \R\) esiste una \hyperref[sec:orgb157329]{rete neurale} che la approssima.
\end{prop}
...
\begin{itemize}
\item input: \(x \in \R\);
\item un \hyperref[sec:orga494515]{layer nascosto} con \(N\) neuroni con funzione di attivazione \(\cos\), con peso dall'input \(w_{j}\) e bias \(b_{j}\);
...
\begin{thm}
Sia \(f \in L^{1}(\R)\)\footnote{Vedi ``\href{../../../../../org/roam/20250624162220-spazi_lp.org}{Spazi Lp}''\label{orgd7cdc78}}. Lo \href{../../../../../org/roam/20250630122400-span.org}{span} di \(\set{T_{\theta}f\mid \theta \in \R}\) è \href{../../../../../org/roam/20250301193045-sottoinsieme_denso.org}{denso} in \(L^{1}(\R)\) se e solo se la \href{../../../../../org/roam/20250630121906-trasformata_di_fourier.org}{trasformata di Fourier}:
\begin{equation*}
...
\begin{thm}
Sia \(f \in L^{2}(\R)\)\textsuperscript{\ref{orgd7cdc78}}. Lo \href{../../../../../org/roam/20250630122400-span.org}{span} di \(\set{T_{\theta}f\mid \theta \in \R}\) è \href{../../../../../org/roam/20250301193045-sottoinsieme_denso.org}{denso} in \(L^{2}(\R)\) se e solo se gli zeri della \href{../../../../../org/roam/20250630121906-trasformata_di_fourier.org}{trasformata di Fourier}\footnote{Vedi ``\href{../../../../../org/roam/20250630122745-proprieta_vera_quasi_ovunque.org}{Proprietà vera quasi ovunque}''}
\begin{equation*}
...
La funzione \(G(x)\) è l'output di una \hyperref[sec:orgb157329]{rete neurale} con un \hyperref[sec:orga494515]{layer nascosto} di \(N\) neuroni (e neurone di output lineare), dove i \hyperref[sec:org5e0fef6]{neuroni} del layer nascosto hanno funzione di attivazione \(f\).
...
Si noti che le seguenti \hyperref[sec:orgb35bde6]{funzioni di attivazione} soddisfano le ipotesi dei Teoremi \ref{teo:wt1} e \ref{teo:wt2}.
\begin{description}
