...
\label{sec:org36794a4}

\section{Neurone Artificiale}
\label{sec:org0026bd1}
...
\label{sec:org0042486}
Nel Machine Learning le funzioni che agiscono nei \hyperref[sec:org0026bd1]{neuroni} vengono dette \uline{funzioni di attivazione}. Se ne presentano alcuni esempi, con i loro nomi specifici.

Sono tutte funzioni \(A \subseteq\R\to B \subseteq \R\).
\subsection{Funzioni Lineari}
\label{sec:org8352580}
...
\label{sec:orgb6c9bc4}

\subsubsection{Threshold step function}
\label{sec:orgeb5bbf0}
...
\label{sec:org301ee17}

\subsubsection{Funzione Logistica}
\label{sec:org6886527}
...
\label{sec:org5432c98}

\subsubsection{Gaussiana}
\label{sec:orgc4e232f}
...
\end{equation*}
dove \(\operatorname{ReLU}\) è la versione multidimensionale della \hyperref[sec:orgbc4b1b1]{funzione di attivazione ReLU}, ovvero
\begin{equation*}
...
\label{sec:orgb8d13fd}
Per evitare il fenomeno dell'\href{../../../../../org/roam/20250627103519-overfitting.org}{overfitting}, è bene mantenere i parametri \uline{piccoli}. Pertanto, data una \hyperref[sec:orgc8dc08a]{funzione costo} \(C(\bm{w})\), la si \uline{regolarizza}, utilizzando una funzione costo \(G(\bm{w})\), data da \(C(\bm{w})\) più un termine di regolarizzazione.
...
\label{sec:org25098bf}
L'apprendimento di una \hyperref[sec:org1346ff2]{rete neurale} è il processo di ricerca dei parametri ottimali per approssimare la funzione target. Questo è un processo iterativo algoritmico, che genera una \href{../../../../../org/roam/20250206170922-sequenze_e_stringhe.org}{sequenza} \((w_{t})\) di parametri. Poiché si parla di numeri immensi di elementi in questa sequenza, spesso ci si riferisce a \(t\) come una sorta di variabile temporale continua.
...
Si noti, inoltre, che se \(\sigma\) è la \hyperref[sec:org0042486]{sigmoide} logistica, allora la derivata massima è ottenuto in \(x=0\) e vale
\begin{equation*}
...
\begin{thm}
Sia \(N\ge {1}\) un intero fissato, e si consideri una \hyperref[sec:org1346ff2]{rete neurale} con un \hyperref[sec:org239d5ff]{layer nascosto} tale che:
\begin{enumerate}
\item l'input della rete sia una variabile reale \(x \in [a,b]\);
\item l'output della rete sia un \hyperref[sec:org0026bd1]{neurone} unidimensionale con \hyperref[sec:org0042486]{funzione di attivazione lineare} e zero bias;
\item ci sono \(N\) neuroni nel layer nascosti con una funzione di attivazione differenziabile tale che \(|\sigma'|<\lambda<1\);
\item i pesi soddisfano la condizione di \hyperref[sec:orgb8d13fd]{regolarizzazione}:
\begin{equation*}
...
\begin{prop}
Anche per una \hyperref[sec:org1346ff2]{rete neurale} composta da un unico \hyperref[sec:org0026bd1]{neurone}, con output
\begin{equation*}
f_{\bm{w},b}(\bm{x}) = \sigma(\bm{w}\cdot\bm{x} + b)
\end{equation*}
dove \(\sigma\) è la \hyperref[sec:org6886527]{funzione logistica}, con pesi \(\bm{w} \in \R^{n}, b \in \R\) e input \(\bm{x} \in I_{n} \coloneqq [0,1]^{n}\), esiste una \href{../../../../../org/roam/20250103103252-funzione_continua.org}{funzione continua} \(g:I^{n}\to \R\) che può essere approssimata. Si supponga che \(\norma{\bm{w}}\le 1\).
\end{prop}
...
\begin{itemize}
\item input: \(x \in \R\);
\item un \hyperref[sec:org239d5ff]{layer nascosto} con \(N\) neuroni con funzione di attivazione \(\cos\), con peso dall'input \(w_{j}\) e bias \(b_{j}\);
...
La funzione \(G(x)\) è l'output di una \hyperref[sec:org1346ff2]{rete neurale} con un \hyperref[sec:org239d5ff]{layer nascosto} di \(N\) neuroni (e neurone di output lineare), dove i \hyperref[sec:org0026bd1]{neuroni} del layer nascosto hanno funzione di attivazione \(f\).
...
Si noti che le seguenti \hyperref[sec:org0042486]{funzioni di attivazione} soddisfano le ipotesi dei Teoremi \ref{teo:wt1} e \ref{teo:wt2}.
\begin{description}
...
\label{sec:org66c6526}

\section{Risultati preliminari}
\label{sec:org8bf2acd}
...
\begin{oss}
La funzione \(c(x)\) di cui sopra è la funzione output di una \hyperref[sec:org1346ff2]{rete neurale} con un layer nascosto:
\begin{itemize}
...
\begin{oss}
In particolare, tutte le \hyperref[sec:org0042486]{funzioni di attivazione} \href{../../../../../org/roam/20250625110110-funzione_sigmoidale.org}{sigmoidali} \(\varphi\) rispettano le ipotesi.
\end{oss}
...
\label{sec:org61d5d34}

\begin{lem}
La \hyperref[sec:org0042486]{funzione di attivazione} \emph{\hyperref[sec:org76b44ce]{softplus}} \(\operatorname{sp}(x)\) è data dalla \href{../../../../../org/roam/20250703105424-prodotto_di_convoluzione.org}{convoluzione}:\footnote{Vedi la \hyperref[sec:orgbc4b1b1]{funzione \(\operatorname{ReLU}\)}}
\begin{equation*}
\operatorname{sp}(x) = \convolution{\operatorname{ReLU}}{K}(x) = \int_{-\infty}^{+\infty} \operatorname{ReLU}(\tau) K(x-\tau)\dif\tau
\end{equation*}
dove \(K(x) = \frac{1}{(1+e^{x})(1+e^{-x})} = \sigma'(x)\)\footnote{Dove \(\sigma(x)\) è la \hyperref[sec:org6886527]{Funzione Logistica}.}.
\end{lem}
...
\begin{lem}
Si definisca ora la \hyperref[sec:org76b44ce]{funzione softplus} scalata:
\begin{equation*}
...
\begin{lem}
Sia \(I_{n}\coloneqq[0,1]^{n}\), e si consideri \(C(I_{n})\)\footnote{Vedi ``\href{../../../../../org/roam/20250113125602-classe_c_di_una_funzione.org}{Classe C di una funzione}''\label{org44be9d0}} dotato della \href{../../../../../org/roam/20250103145124-topologia.org}{topologia} \href{../../../../../org/roam/20250301193530-topologia_indotta_da_una_distanza.org}{indotta} dalla \href{../../../../../org/roam/20250301193511-spazio_metrico.org}{metrica}
\begin{equation*}
...
\end{equation*}
Si vedano ``\href{../../../../../org/roam/20250624162220-spazi_lp.org}{Spazi Lp}''\label{org8c22223}} e
\begin{equation*}
...
\end{prop}
\begin{thm}
Ogni funzione in \(L^{q}(\R^{n})\)\footnote{Vedi ``\href{../../../../../org/roam/20250624162220-spazi_lp.org}{Spazi Lp}''} può essere approssimata arbitrariamente bene nella norma \(\norma{\cdot}_{q}\) da una \hyperref[sec:org1346ff2]{rete neurale} \hyperref[sec:org9afed3b]{feedforward} \hyperref[sec:orgbc4b1b1]{ReLU}.
\end{thm}
\subsection{Accuratezza dell'approssimazione}
\label{sec:orgb4d206b}

Si consideri una \hyperref[sec:org1346ff2]{rete neurale} con un \hyperref[sec:org239d5ff]{layer nascosto} contenente \(N\) \hyperref[sec:org0026bd1]{neuroni}, che produce un output \(G(x)\), che vuole \href{../../../../../org/roam/20250630103725-funzioni_approssimate_da_una_rete_neurale.org}{approssimare} \(f\) in uno spazio funzionale quale\footnote{Sia \(I_{n}\coloneqq[0,1]^{n}\). Vedi
\begin{itemize}
...
\label{sec:org571e6f4}

\section{Exact Learning (Machine Learning)}
\label{sec:org3a813f6}
By exact learning we mean the expressibility of a \hyperref[sec:org1346ff2]{network} to reproduce exactly the desired target function, i.e. the output of the network is exactly the target function. For an exact learning the network weights do not need \hyperref[sec:org25098bf]{tuning}; their values can be found exactly. Si parla quindi di \uline{rappresentazione esatta}, e non di \uline{apprendimento}.
...
\label{sec:orgad5bed8}
Si consideri a titolo esplicativo \(C([0,1])\)\footnote{Vedi ``\href{../../../../../org/roam/20250113125602-classe_c_di_una_funzione.org}{Classe C di una funzione}''}, e si consideri una \hyperref[sec:org1346ff2]{rete neurale} con un \hyperref[sec:org239d5ff]{layer nascosto} con \hyperref[sec:org0042486]{funzione di attivazione} \hyperref[sec:org6886527]{logistica} \(\sigma\). La \hyperref[sec:org1346ff2]{funzione di input-output della rn} è
\begin{equation*}
...
\label{sec:orgf7b7fa7}

\begin{prop}
La funzione \(\max\set{x_{1},x_{2}}\) è \hyperref[sec:org3a813f6]{rappresentata esattamente} da una \hyperref[sec:org1346ff2]{rete neurale} con un \hyperref[sec:org239d5ff]{layer nascosto} con 4 \hyperref[sec:org0026bd1]{neuroni} \hyperref[sec:orgbc4b1b1]{ReLU}, ovvero
\begin{equation*}
...
\begin{prop}
La funzione \(f(x_{1},\dots,f_{x_{n}}) = \max\set{x_{1},\dots,x_{n}}\) è \hyperref[sec:org3a813f6]{rappresentata esattamente} da una \hyperref[sec:org1346ff2]{rete neurale} \href{../../../../../org/roam/20250624155858-neurone_artificiale.org}{ReLU-feedforward} con
\begin{equation*}
L=\begin{cases}
2k & \text{se }n=2^{k}\\
...
\begin{prop}
Esiste una \hyperref[sec:org1346ff2]{rete neurale} \hyperref[sec:org9afed3b]{feedforward} con input di dimensione 2, un layer nascosto di larghezza 2, output di dimensione 1 con \hyperref[sec:org0042486]{funzione di attivazione} \hyperref[sec:orgbc4b1b1]{ReLU} in ogni \hyperref[sec:org0026bd1]{neurone} che \hyperref[sec:org3a813f6]{rappresenta esattamente} la funzione
\begin{equation*}
